...
import { Requests as TodayRequests } from "./Requests/requests";
import { Reviews } from "./Reviews/reviews";
import { fetchUserPhones, fetchUserChatsExtranet } from "@/redux/thunk/user";
...
export const Today = () => {
    ...
    const [value, setValue] = React.useState('leaving');
    const [isLoaded, setIsLoaded] = React.useState(false);
    const [all, setAll] = React.useState<any>(false);
    let router = useRouter();

    useEffect(() => {
        Promise.all([dispatch(fetchBookingsExtranet({status: '', query: ''})), dispatch(fetchReviewsExtranet()), dispatch(fetchUserPhones()), dispatch(fetchUserChatsExtranet())])
    ...
    return (
        <>
            ...
            {isLoaded ? (
                ...
                >
                    <Typography variant="h5" component="h1">Сегодня</Typography>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 56 }}>
                        ...
                        <Link style={{ color: 'black', textDecoration: 'none' }} href={'/host/booking-request/all'}>Все бронирования ({all?.filter((item: any) => item.status !== BookingStatuses.created).length})</Link>
                    </div>
