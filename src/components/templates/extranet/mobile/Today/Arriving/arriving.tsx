            ...
            {isLoaded ? (
                <>
                    {state.booking.bookings.length !== 0 ? (
                        <List uploadRequest={() => dispatch(fetchBookingsExtranet({status: 'eq:confirmed', query: `&start_date[]=gte:${dateForRequestFormat(new Date())}&start_date[]=lte:${dateForRequestFormat(addDays((new Date()), 1))}`}))} />
                    ) : (
