    ...
    return (
        <>
            <Card variant="outlined" sx={{ borderRadius: "18px" }}>
                <CardActionArea onClick={() => handleOpenModalWindowDetails(props)}>
                    <CardMedia
                        component="img"
                        height="170px"
                    ...
                    <CardContent sx={{ pb: 0 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Typography variant="h6" component="h3" sx={{ width: 'calc(100% - 150px)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {props.property.name}
                            </Typography>
                            <Typography variant="body2" color="text.secondary" gutterBottom>
                                {BookingStatusesViewSecure[props.status as keyof typeof BookingStatusesViewSecure]}
                            </Typography>
                        </div>
                        <Typography variant="body2" color="text.secondary" gutterBottom>
                        ...
                        <Typography variant="subtitle1" gutterBottom>
                            {rangeStartDateEndDate(props.start_date, props.end_date)}
                        </Typography>
                ...
                <CardActions sx={{ justifyContent: 'space-between' }}>
                    ...
                    {(+formatDistanceStrict(new Date(), new Date(props.end_date), { unit: 'day' }).split(' ')[0] < 90 && props.status === BookingStatuses.finished && !props.review) &&
                        <Link href={`/guest/reservations/${props.id}/review`}><Button sx={{ mt: 0.5, mb: 0.5, color: 'black', borderColor: 'black' }} size="small" variant="outlined">Оставить отзыв</Button></Link>
                    }
                </CardActions>
            </Card>
