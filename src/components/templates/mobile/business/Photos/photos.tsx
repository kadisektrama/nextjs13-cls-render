import React, { useState } from "react"
import ImageGallery from "react-image-gallery";
import photos from "./photos.module.scss";
import { useAppSelector } from "@/redux/hooks/hooks";

export const Photos: React.FC = () => {
    const property = useAppSelector(state => state.property.property)
    const [imageUrls] = useState<any>([]);

    property.photos.forEach((element: any) => {
        function myRenderItem () {
            return <>
                <img className={photos.imageAspectRatio} style={{ maxHeight: '100%', maxWidth: '100%', width: '100%', aspectRatio: 5 / 3, objectFit: 'cover' }} src={`${element['url']}?width=480`} alt={property.name}/>
            </>;
        }

        imageUrls.push({
            original: `${element['url']}?width=480`,
            thumbnail: `${element['url']}?width=480`,
            originalAlt: property.name,
            thumbnailAlt: property.name,
            renderItem: myRenderItem,
        })
    })

    return(
        <>
            {imageUrls.length !== 0
                ? <ImageGallery showThumbnails={false} showPlayButton={false} items={imageUrls} />
                : <img
                    style={{ width: '100%', aspectRatio: 5 / 3, objectFit: 'none' }}
                    className="image-gallery-image"
                    src='data:image/png;base64,'
                    title="hello world"
                    alt={'Nothing'}
                />
            }
        </>
    )
}


